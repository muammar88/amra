...
class Modal extends CI_Controller
{
   ...
   public function __construct()
   {
      parent::__construct();
      # Load user model
      $this->load->model('Model_modal', 'model_modal');
   ...
   # get filter modal
   function get_filter_modal()
   {
      $error = 0;
      $error_msg = '';
      # list periode
      $list_periode = $this->model_modal->get_list_periode();
      # filter
   ...
   function _ck_periode_exist($id)
   {
      if( $id == 0 ) {
         ...
         if ($this->model_modal->check_periode_exist($id)) {
            return TRUE;
         } else {
   ...
   function daftar_modal()
   {
     ...
     */
      if ($this->form_validation->run()) {
         ...
         $list = $this->model_modal->get_index_modal($param);
         if (count($list) > 0) {
            $return = array(
               'error'   => false,
               'error_msg' => 'Data modal berhasil ditemukan.',
               'list' => $list,
               $this->security->get_csrf_token_name() => $this->security->get_csrf_hash()
            );
         } else {
            $return = array(
               'error'   => true,
               'error_msg' => 'Data modal tidak ditemukan.',
               $this->security->get_csrf_token_name() => $this->security->get_csrf_hash()
            );
         }
      } else {
